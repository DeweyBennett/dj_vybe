...
export default function CompletePage() {
  ...
  return (
    <div id="payment-status">
      <div id="status-icon" style={status == 'succeeded' ? {backgroundColor: '#30B130'} : status == 'processing' ? {backgroundColor: '#6D6E78'} : status == 'requires_payment_method' ? {backgroundColor: '#DF1B41'} : {backgroundColor: '#DF1B41'}}>
        {status == 'succeeded' ? SuccessIcon : status == 'processing' ? InfoIcon : status == 'requires_payment_method' ? ErrorIcon : ErrorIcon}
      </div>
      <h2 id="status-text">{status == 'succeeded' ? "Payment succeeded" : status == 'processing' ? "Your payment is processing." : status == 'requires_payment_method' ? "Your payment was not successful, please try again." : "Something went wrong, please try again."}</h2>
      ...
      {intentId && <Link href={`https://dashboard.stripe.com/payments/${intentId}`} id="view-details" target="_blank">View details
